...
export default async function AdminPage() {
  ...
  const [
    ...
    db.business.findMany({
      ...
      take: 6,
      select: { slug: true, name: true, city: true, status: true, createdAt: true },
    }),
  ]);
          ...
          <div className="rounded-2xl border border-border bg-surface p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link
                ...
                className="text-sm text-terracotta-deep hover:underline underline-offset-4"
              >
                See all →
              </Link>
            </div>
            <ul className="divide-y divide-border">
              {recentBusinesses.map((b) => (
                <li key={b.slug} className="py-3 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <Link
                      href={`/b/${b.slug}`}
                      className="font-semibold text-ink hover:text-terracotta-deep block truncate"
                    >
                      {b.name}
                    </Link>
                    <p className="text-xs text-ink-soft">
                      {b.city} · {b.status.toLowerCase()} · {b.createdAt.toLocaleDateString()}
                    </p>
                  </div>
                </li>
              ))}
